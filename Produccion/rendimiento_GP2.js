...
(async function init() {
  ...
  // GP2: un solo RPC trae la lista de matrices (con Tiempo_Historico) y el mapa de empleados.
  // No existe tabla Empleados en GP2: el nombre del operario viene embebido en produccion.
  const { data: bundle, error: bundleErr } = await sb.rpc("produccion_bundle");
  if (bundleErr) { console.error(bundleErr); $loading.textContent = "Error cargando matrices: " + (bundleErr.message || bundleErr); return; }

  matrices = (bundle.matrices || []).filter(m => m.N_Matriz != null);
  ...
  const empObj = bundle.empleados || {};
  Object.keys(empObj).forEach(leg => {
    empleadosMap[String(leg).trim()] = empObj[leg] || "";
  });
